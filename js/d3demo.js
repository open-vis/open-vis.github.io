var div = document.getElementsByClassName('video-wrapper')[0];

var width = div.clientWidth,
    height = div.clientHeight,
    height_div_25 = height / 25,
    height_div_16 = height / 16,
    width_div_48 = width / 48,
    m = [height_div_25, width_div_48, height_div_16, width_div_48],
    w = width - m[1] - m[3],
    h = height - m[0] - m[2],
    w_div_16 = width / 16,
    running = false,
...
function lines() {
    running = true;
    x = d3.time.scale().range([0, w - w_div_16]);
    y = d3.scale.linear().range([h / 4 - height_div_25, 0]);
...
function horizons() {
    ...
    g.select("circle").transition()
        .duration(duration)
        .attr("transform", function (d) {
            return "translate(" + (w - w_div_16) + "," + (-h / 4) + ")";
        })
        .remove();
    ...
    g.select("text").transition()
        .duration(duration)
        .attr("transform", function (d) {
            return "translate(" + (w - w_div_16) + "," + (h / 4 - height_div_25) + ")";
        })
        .attr("dy", "0em");
    ...
    g.each(function (d) {
        ...
        d3.select(this).selectAll(".area")
            ...
            .attr("class", "area")
            .attr("transform", function (d) {
                return "translate(0," + (d * (h / 4 - height_div_25)) + ")";
            })
...
function stackedArea() {
    ...
    t.select("text")
        .attr("transform", function (d) {
            ...
            return "translate(" + (w - w_div_16) + "," + y(d.price / 2 + d.price0) + ")";
        });
...
function overlappingArea() {
    ...
    t.select("text")
        ...
        .attr("transform", function (d) {
            d = d.values[d.values.length - 1];
            return "translate(" + (w - w_div_16) + "," + y(d.price) + ")";
        });
...
function stackedBar() {
    x.rangeRoundBands([0, w - w_div_16], .1);
